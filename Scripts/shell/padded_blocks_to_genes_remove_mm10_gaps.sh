...
#SBATCH --job-name=chr_pad_blocks_to_genes_remove_mm10_gaps
...
# Usage
# -----------------------------------------
# sbatch chr_pad_blocks_to_genes_remove_mm10_gaps.sh chrY
# sbatch chr_pad_blocks_to_genes_remove_mm10_gaps.sh chr21
# -----------------------------------------

CHR=$1

if [ -z "$CHR" ]; then
    echo "Usage: sbatch chr_pad_blocks_to_genes_remove_mm10_gaps.sh chrY"
    exit 1
fi
...
BLOCKS_DIR=$BASE/Results/${CHR}_chunks_padded_rodents_mm10ref
OUT_FASTA_DIR=$BASE/Results/gene_level_fastas_mm10ref/${CHR}
...
python $BASE/Data/Scripts/padded_blocks_to_genes_remove_mm10_gaps.py \
    --gtf "$GTF" \
